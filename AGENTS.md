...
This project tests whether a model can learn to reason more efficiently by training on corrected local transitions extracted from reasoning traces generated by the current model version itself: `question + useful reasoning prefix → next useful reasoning step`. Code runs as uv-managed Python utilities, a CLI, and Hugging Face Jobs scripts. Serious dataset creation and training are designed for Hugging Face cloud infrastructure; local execution is for tests, config checks, dry runs, and script validation. **This is an R&D project — the pipeline evolves rapidly, and pipeline inspection is the primary quality feedback loop.**

## R&D Workflow — Pipeline Inspection Is the Real Feedback Loop

The pipeline changes rapidly: D prompts, unit splitting strategies, G model checkpoints, context-advance logic, and training configs all evolve as experiments run. There is no stable "known-good" baseline to protect; every change is an experiment, and every experiment must be observed.

**Pipeline inspection** is the practice of running the live pipeline against a real model and reading every step: G's generated trace, the unit split, D's removal decision, the emitted `input_x → target_y` row, and the next context. Automated tests only verify that the pipeline is wired and runs — they cannot tell you whether G's reasoning is useful, whether the unit split is clean, or whether D is picking genuine filler.

### When pipeline inspection is required

After any change that can affect data quality, run a pipeline inspection before the change is considered done:

- G or D model configuration
- Unit splitting strategy or parameters
- D prompt version (`prompts/*.txt`)
- Context-advance logic (`advance_context_units`)
- Row-building logic (`build_pruning_transition_row`)
- Question loading or formatting (`load_questions`, `format_spectrum_question`)
- Retry or depth control logic
- Public loop signatures (`build_rows_for_question`, `build_pt_dataset`)
...
| `src/reasoning_pruning/pipeline_inspection.py` and `scripts/pipeline_inspection.py` | A printing `PruningObserver` that hooks the **production** `build_rows_for_question` loop, plus the thin CLI that runs it. It does not reimplement the loop — printing is an observer on the real loop, so inspection output can never drift from what data creation runs. This is the real post-change check (see "Testing & Pipeline Inspection"). |
...
| `configs/data/*.yaml` | Dataset-builder configs plus pipeline inspection configs. |
...
- Using the base model (`google/gemma-4-E2B-it`) as G after round 1. After the first training run, G must always be the fine-tuned model (`avreymi/gemma-4-E2B-it-reasoning-pruning`). Data generated by the base model teaches the fine-tuned model to skip the base model's habits, not its own.

## Testing & Pipeline Inspection

Run the automated suite to verify the pipeline is wired and runs (see the `testing` skill for what to test):

```bash
uv run pytest
uv run python -m py_compile src/reasoning_pruning/*.py scripts/*.py
uv run python scripts/pipeline_inspection.py --help
...
**For any pipeline-affecting change, pipeline inspection is required — see "R&D Workflow" above.** The inspector hooks the production loop via `PruningObserver`; what it prints is exactly what data creation runs. Never reintroduce a separate inspection loop. The `--dry-run` command only works with Gemini/local-file configs; Gemma-4 requires HF Jobs or the Colab notebook.

## Iterative Self-Distillation — G Is Always the Current Fine-Tuned Model

**G must always be the most recently trained fine-tuned model from this repo, never the base model.**

The self-distillation cycle works iteratively:
- Round 1: G = `google/gemma-4-E2B-it` (base) → data → training → `avreymi/gemma-4-E2B-it-reasoning-pruning`
- Round 2 onward: G = `avreymi/gemma-4-E2B-it-reasoning-pruning` → data → training → next checkpoint

Using the base model as G in round 2+ would generate traces shaped by the base model's habits, not the fine-tuned model's. The training rows would then teach the fine-tuned model to skip patterns it never produces — the data would be misaligned. G must always be the same model that will be trained on the resulting rows.

The active generator model is **`avreymi/gemma-4-E2B-it-reasoning-pruning`** (current fine-tune, round 2+).

## Dataset creation and training

Dataset creation runs on HF Jobs with `scripts/create_dataset_gemma4_job.py`; it requires `HF_TOKEN` and `GEMINI_API_KEY`. The active dataset-builder configs use `avreymi/gemma-4-E2B-it-reasoning-pruning` as G and Gemini Flash Lite as D.
